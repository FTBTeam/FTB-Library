...
public enum LazyIconRenderer implements IconRenderer<LazyIcon> {
    ...
    @Override
    public void render(LazyIcon icon, GuiGraphicsExtractor graphics, int x, int y, int w, int h) {
        IconHelper.renderIcon(icon.getDelegate(), graphics, x, y, w, h);
    }

    @Override
    public void renderStatic(LazyIcon icon, GuiGraphicsExtractor graphics, int x, int y, int w, int h) {
