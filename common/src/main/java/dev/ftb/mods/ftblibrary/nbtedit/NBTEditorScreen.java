	...
	public class ButtonBasicTag extends ButtonNBT {
		...
		public void edit() {
			switch (nbt.getId()) {
				...
				case Tag.TAG_SHORT ->
						openEditOverlay(new IntConfig(Short.MIN_VALUE, Short.MAX_VALUE), ((NumericTag) nbt).getAsInt());
				case Tag.TAG_INT ->
				...
				case Tag.TAG_DOUBLE, Tag.TAG_ANY_NUMERIC ->
		...
		public void onCallback(ConfigValue<?> value, boolean accepted) {
			if (accepted) {
				switch (nbt.getId()) {
					case Tag.TAG_BYTE ->
							nbt = ByteTag.valueOf(((Number) value.getValue()).byteValue());
					case Tag.TAG_SHORT ->
							nbt = ShortTag.valueOf(((Number) value.getValue()).shortValue());
					case Tag.TAG_INT ->
							nbt = IntTag.valueOf(((Number) value.getValue()).intValue());
					case Tag.TAG_LONG ->
							nbt = LongTag.valueOf(((Number) value.getValue()).longValue());
					case Tag.TAG_FLOAT ->
							nbt = FloatTag.valueOf(((Number) value.getValue()).floatValue());
					case Tag.TAG_DOUBLE, Tag.TAG_ANY_NUMERIC ->
